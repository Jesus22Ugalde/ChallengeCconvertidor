...
public class VistaConvertidorMedidas extends JFrame implements ActionListener {
    JLabel from, to;
    JComboBox fromCombo, toCombo;
    ArrayList<String> currencyOptions = new ArrayList<>(Arrays.asList("CM", "M", "KM"));
    JButton convert, exit;
    AdministradorMedidas AdministradorMedidas = new AdministradorMedidas();

    VistaConvertidorMedidas() {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (source.equals(convert)) {
            ...
            if(measure != 0) {
                Double val = this.AdministradorMedidas.convert(
                        (String) fromCombo.getSelectedItem(),
                        (String) toCombo.getSelectedItem(),
                        measure
                );
